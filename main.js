...
async function startTiny(callback) {
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            let flags = [
                path.join(helperResourcePath, 'tap-windows6', 'tap-windows-9.9.2_3.exe'),
                '/S',
                'certuril',
                '-addstore',
                `'TrustedPublisher'`,
        ...
        core.on('close', (code, signal) => {
            ...
            if (code && code != 0) {
                ...
                coreInterrupt = true
                core = null
                dialog.showErrorBox('Error', util.format('Failed to start the Core, see "%s" for more details.'))
                return resolve()
            }
...
async function checkForUpdate(win) {
    return new Promise(function (resolve, reject) {
        ...
        function sendStatusToWindow(text) {
            ...
            try {
                win.webContents.executeJavaScript('document.body.innerHTML = "' + text + '";')
            } catch (e) {
            }
        }
